...
export default function Header() {
    ...
    const openMenu = () => {
        ...
        if (
          document.body.classList.contains("opacity")
        ) {
  ...
  return (
    <header className={`${styles.header}`}>
        <nav className='py-2 px-4 flex justify-between min-h-[80px]'>
            ...
            <button id="button" onClick={openMenu} className={`${!toggle ? "" : styles.open}`}>
                <span></span>
            </button>
            <ul className={`flex items-center gap-4 ${!toggle ? "" : styles.active}`}>
